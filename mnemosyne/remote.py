...
class RemoteFS:
    ...
    def init(remote):
        if remote.type == "cifs":
            return RemoteCIFS(remote)
        elif remote.type == "block":
            return RemoteBlock(remote)
        else:
            raise RuntimeError(f"Don't know remote type {remote.type}")
...
class RemoteBlock:
    ...
    def mount(self):
        ...
        logger.info("Mounting block device...")

        proc = subprocess.run(
            [
                "mount", self.remote.device, self.mnt
            ]
        )
